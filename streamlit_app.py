...
# Define skills and weights for each domain
skills = {
...
if uploaded_files and selected_role:
    ...
    for uploaded_file in uploaded_files:
        ...
        doc_nlp = nlp(extracted_text)
        cleaned_text = " ".join([token.lemma_ for token in doc_nlp if not token.is_stop and token.is_alpha])

        # Match skills and calculate score
        matched_keywords = {}
        total_score = 0
        for skill, weight in skills[selected_role].items():
    ...
    sorted_data = sorted(resume_data, key=lambda x: x["Score"], reverse=True)

    # Display results
    st.subheader("Ranked Resumes")
    st.table(pd.DataFrame(sorted_data))

    # Save to Supabase
    if st.button("Save Rankings"):
        try:
            response = supabase.table("resume_rankings").insert(sorted_data).execute()
